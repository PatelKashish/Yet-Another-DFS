...
def load_args():
    ...
    if args.NUM_LOAD == 0:
        print("DFS has been loaded for the first time. Please clean using the `clean` command")
        _input = None
        while _input != "clean":
            _input = input("(yadfs_309_312_330_368) > ")
            if _input == "clean":
                format_namenode_datanode(quit=False)
            else:
                print("Please enter `clean` to clean the DFS")
...
def format_namenode_datanode(quit=True, *vargs):
    ...
    print("Name Node and Data Nodes cleaned successfully. Please restart.")
    #if quit:
     #   exit()
...
def create_namenode_checkpoints():
    ...
    checkpoint_path = Path(args.NAMENODE_CHECKPOINTS_PATH).joinpath(checkpoint_name)
    checkpoint_path.mkdir(parents=True)

    primary_namenode_path = Path(args.PATH_TO_NAMENODES).joinpath(args.PRIMARY_NAMENODE_NAME)
    files_in_primary_namenode = primary_namenode_path.glob('**/*')
    files_in_primary_namenode = list(filter(Path.is_file, files_in_primary_namenode))
    ...
    if check_path_exists_in_hdfs(path):
        ...
        if components[-1] in current:
            if current[components[-1]] is None:
                ...
                print(
                    f"Error: Path {path} is a directory, not a file. Use `destroy` instead.")
        else:
            print(f"Error: Path {path} not found.")
            return False
    else:
    ...
    if check_path_exists_in_hdfs(path):
        ...
        if components[-1] in current:
            if not current[components[-1]] is None:
                ...
                print(
                    f"Error: Path {path} is a file, not a directory. Use del instead.")
        else:
            print(f"Error: Path {path} not found.")
            return False
    else:
            ...
            for i in range(1, num_blocks+1):
                ...
                for j in range(args.REPLICATION_FACTOR):
                    ...
                    if datanode_id is None:
                        ...
                        log_datanode(
                            datanode_id, destination_block_name, "load")
...
def cat(*vargs):
    ...
    if check_path_exists_in_hdfs(file_path):
        ...
        for block_id in block_id:
            ...
            log_datanode(datanode_id, block_id, "show")
...
def download(*vargs):
    hdfs_file_path, local_destination = vargs

    if check_path_exists_in_hdfs(hdfs_file_path):
        file_id = get_file_id_from_hdfs_file_path(hdfs_file_path)
        if file_id is None:
            print("Error: File not found.")
            return False
        num_blocks = args.FILE_INFO[file_id]['num_blocks']
        block_ids = [f"{file_id}__{bid}" for bid in range(1, num_blocks + 1)]
        block_contents = []

        for block_id in block_ids:
            datanode_id = get_datanode_id_from_block_id(block_id)
            log_datanode(datanode_id, block_id, "retrieve")
            block_path = Path(args.PATH_TO_DATANODES).joinpath(datanode_id).joinpath(block_id)

            with open(block_path, "r") as f:
                block_contents.append(f.read())

        file_content = '\n'.join(block_contents)
        with open(local_destination, "w") as local_file:
            local_file.write(file_content)

        return True
    else:
        print(f"File {hdfs_file_path} not found.")
        return False
load_args()

def mv(source_path, destination_path):
    if not check_path_exists_in_hdfs(source_path):
        print(f"Error: Source file {source_path} not found.")
        return False

    if check_path_exists_in_hdfs(destination_path):
        print(f"Error: Destination file {destination_path} already exists.")
        return False

    file_id = get_file_id_from_hdfs_file_path(source_path)
    if file_id is None:
        print(f"Error: File not found.")
        return False

    args.FILE_INFO[file_id]['file_path'] = destination_path
    update_namenode_file_info()

    components_source = source_path.split('/')
    components_source = list(filter(bool, components_source))
    current_source = args.FILESYSTEM
    for component in components_source[:-1]:
        if component in current_source:
            current_source = current_source[component]
    del current_source[components_source[-1]]

    components_destination = destination_path.split('/')
    components_destination = list(filter(bool, components_destination))
    current_destination = args.FILESYSTEM
    for component in components_destination[:-1]:
        if component not in current_destination:
            current_destination[component] = {}
        current_destination = current_destination[component]
    current_destination[components_destination[-1]] = None
...
def cp(sp_dfs, dp_dfs):
    temp_local_path = "temp_file.txt"  # You can choose any temporary file name

    try:
        download(sp_dfs, temp_local_path)
        put(temp_local_path, dp_dfs)
    finally:
